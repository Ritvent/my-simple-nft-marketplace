...
module nft::test_sui_nft_marketplace {
    ...
    public struct TestChainNFT has key, store {
        id: UID,
        /// Name for the token
        name: string::String,
    ...
    entry fun burn_nft(nft: TestChainNFT) {
        let TestChainNFT { id, name: _, description: _, url: _ } = nft;
        object::delete(id)
    }
    ...
    entry fun buy_nft(
        ...
        transfer::public_transfer(nft, buyer);  // Transfer NFT to buyer
    ...
    public fun burn(nft: TestChainNFT) {
        let TestChainNFT { id, name: _, description: _, url: _ } = nft;
        object::delete(id)
    }
    ...
    /// Get the NFT's name
    public fun name(nft: &TestChainNFT): &string::String {
        &nft.name
    }

    /// Get the NFT's description
    public fun description(nft: &TestChainNFT): &string::String {
        &nft.description
    }

    /// Get the NFT's URL
    public fun url(nft: &TestChainNFT): &Url {
        &nft.url
    }

    /// Get the NFT's ID
    public fun nft_id(nft: &TestChainNFT): ID {
        object::id(nft)
    }
...
module nft::nft_marketplace_tests {
    use nft::nft_marketplace::{Self, TestChainNFT};
    ...
    fun test_mint_and_transfer() {
        ...
        {
            let nft = ts::take_from_sender<TestChainNFT>(&scenario);
    ...
    fun test_update_description() {
        ...
        ts::next_tx(&mut scenario, addr1);
        {
            let mut nft = ts::take_from_sender<TestChainNFT>(&scenario);
